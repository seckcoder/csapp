  ...
  while (fgets(line, MAXBUF, tracefp)) {
    ...
    if (!strcmp(command, "WAIT")) {
      if (readable(syncfd[0], DRIVER_TIMEOUT) == 0) {
        ...
        //printf("runtrace: received sync from job: %s, %d, %d\n", buf, (int)strlen(buf), n);
        // fflush(stdout);
        if (verbose)
    ...
    /* SIGNAL command */
      ...
      // printf("runtrace: sent sync to shell job, %s\n", bufp);
      // fflush(stdout);
      if (verbose)
...
int next_prompt(void)
{
  ...
  // printf("next prompt:%s %d\n", buf, strcmp(buf, PROMPT));
